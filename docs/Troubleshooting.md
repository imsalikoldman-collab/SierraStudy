...
- **Симптом**: `HotSwap.ps1` выводит предупреждение `Copy failed... The process cannot access the file '...\\SierraStudy.dll' because it is being used by another process.` и остановку копирования.
...
- **Решения**:
  ...
  2. Использовать автоматический fallback, который сам отправит команды RELEASE/ALLOW по UDP и скопирует DLL через временный файл:  
     `pwsh -NoProfile -File scripts/HotSwap.ps1 -Dll out\\x64\\Release\\SierraStudy.dll -SierraDataDir C:\\2308\\Data -AutoRemoteFallback`
