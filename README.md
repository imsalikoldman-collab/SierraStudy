...
- **Wrapper** (`projects/Wrapper/src`, `projects/Wrapper/include/sierra/acsil`) — динамическая библиотека-адаптер Sierra Chart ↔ Core:
  - `study.cpp` — опросчик GexBot (SCSFExport `scsf_SierraStudyGexBotPoller`): по символу графика маппит тикер (ES/MES→ES_SPX, NQ/MNQ→NQ_NDX), выполняет GET `https://api.gexbot.com/{TICKER}/state/{GREEK}?key=...` через libcurl, парсит JSON ответ RapidYAML и выводит текстом на графике;
